...
import { getGeoPerformanceLabel } from '@/lib/practice/cityCountryEngine';
import type { PracticeSessionStats } from '@/types/practice';

const LABEL_STYLES: Record<string, string> = {
  'World Atlas': 'text-yellow-600 bg-yellow-50 ring-yellow-300',
  Globetrotter: 'text-brand-700 bg-brand-50 ring-brand-300',
  Navigator: 'text-purple-700 bg-purple-50 ring-purple-300',
  Backpacker: 'text-gray-700 bg-gray-100 ring-gray-300',
  'Lost Tourist': 'text-gray-600 bg-gray-50 ring-gray-200',
};

interface CityCountryCompleteProps {
  stats: PracticeSessionStats;
  onPlayAgain: () => void;
}

export function CityCountryComplete({ stats, onPlayAgain }: CityCountryCompleteProps) {
  const { score, correctCount, totalCount, maxStreak, difficulty } = stats;
  const accuracy = totalCount > 0 ? Math.round((correctCount / totalCount) * 100) : 0;
  const label = getGeoPerformanceLabel(correctCount, totalCount);
  const labelStyle = LABEL_STYLES[label] ?? LABEL_STYLES['Lost Tourist'];
