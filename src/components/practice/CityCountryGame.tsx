...
import { useCityCountry } from '@/hooks';
import { PracticeHUD } from './PracticeHUD';
import { CityCountryQuestionCard } from './CityCountryQuestionCard';
import { ResultFeedback } from './ResultFeedback';
import { CityCountryComplete } from './CityCountryComplete';
import type { CityCountryConfig, PracticeDifficulty, PracticeQuestionCount } from '@/types/practice';
...
interface SettingsPanelProps {
  onStart: (config: CityCountryConfig) => void;
}
...
function SettingsPanel({ onStart }: SettingsPanelProps) {
  ...
  const handleStart = () => {
    onStart({ mode: 'city_country', difficulty, questionCount });
  };
  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        <h1 className="mt-1 text-2xl font-extrabold text-gray-900">Country Finder</h1>
        <p className="mt-2 text-sm text-gray-500">
          Given a city, pick the country it&apos;s in before the clock runs out.
        </p>
      </div>
...
function IntroPanel({ onDone }: { onDone: () => void }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-gray-400">Pick the country each city belongs to</p>
    </motion.div>
  );
}
...
export function CityCountryGame() {
  const practice = useCityCountry();
