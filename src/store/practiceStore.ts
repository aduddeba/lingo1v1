...
interface PracticeActions {
  initSession: (questions: PracticeQuestion[], config: PracticeConfig) => void;
  startQuestion: () => void;
  tick: () => void;
    ...
    immer((set) => ({
      ...
      submitAnswer: (answer) =>
        set((state) => {
          ...
          const correctAnswerLanguage =
            q.type === 'language_ancestry'
              ? (q.languageChain[q.hiddenIndex - 1] ?? '')
              : (q.chain[q.hiddenIndex]?.language ?? '');
          const targetWord =
            q.type === 'language_ancestry' ? q.answer : q.targetWord;
          ...
          state.lastResult = {
            ...
            targetWord,
            targetLabel: q.type === 'language_ancestry' ? 'Language' : 'Evolved into',
            pointsEarned,
